...
;(async () => {
  ...
  if (accept) { await accept.click({ force: true }); await page.waitForTimeout(500); }
  await page.evaluate(() => {
    document.querySelectorAll('#onetrust-consent-sdk, .ot-pc-dark-filter, .ot-sdk-overlay').forEach(el => el.remove());
  });
  ...
  let found = false;
  for (let i = 0; i < 15; i++) {
    const link = await page.$('a.ACategoryOverviewSlider__Link:has-text("Hrana in pijača")');
    if (link) { await link.click({ force: true }); found = true; break; }
    const next = page.locator('div.ux-base-slider button[aria-label="Arrow pointing right"]:not([disabled])');
    if (await next.count()) {
  ...
  for (const title of filters) {
    ...
    await page.click(`span.odsc-link-action__element:has-text("${title}")`, { force: true });
    await page.waitForTimeout(1000);

    const loadMoreBtn = page.locator('div.s-load-more button:has-text("Naloži več izdelkov")').first();
    ...
    const items = await page.$$eval('.odsc-tile', tiles =>
      tiles.map((el) => {
        const rawAction = el
        .querySelector(".ods-price__value")
        ?.innerText.trim() || null;
      const rawPrice = (
