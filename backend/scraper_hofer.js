...
async function getPdfLinks() {
  const INDEX =
    "https://www.hofer.si/sl/ponudba/aktualni-letaki-in-brosure.html";
...
async function extractImagesHtml(browser, slug) {
  ...
  await page.goto(`https://letaki.hofer.si/${slug}/page/1`, {
  ...
  while (true) {
    ...
    const next = await page.$(
      "button.reader-control--next, .flipbook-control-next"
    );
    if (!next) break;
    await Promise.all([
  ...
  for (const slug of slugs) {
    ...
    const docs = raw
      ...
      .map((item) => {
        ...
        const actionP =
          actionStr != null ? parseFloat(actionStr.replace(",", ".")) : null;
        return {
          name: item.name,
          weight: item.weight,
    ...
    for (const doc of docs) {
      ...
      if (!existing) {
        ...
        const initial =
          doc.actionPrice != null ? doc.actionPrice : doc.newPrice;
        ...
        console.log(
          `Updated:  ${doc.name} | ${existing.price} → ${doc.newPrice}`
        );
      }
