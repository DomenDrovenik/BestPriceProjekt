...
test('kosaricaTest', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Ime novega seznama...' }).fill('test1');
  await page.getByRole('button', { name: 'Ustvari seznam' }).click();
  await page.locator('div').filter({ hasText: 'test10 izdelkov' }).nth(3).click();
  await page.getByRole('link', { name: 'Izdelki' }).first().click();
  await page.locator('div:nth-child(5) > .align-middle').first().click();
  await page.getByRole('button', { name: 'Dodaj' }).click();
  await page.getByRole('link', { name: 'Nakupovalni seznam' }).first().click();
  await expect(page.locator('div').filter({ hasText: 'test10 izdelkov' }).nth(3)).toBeVisible();
